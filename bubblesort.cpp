...
void bubbleSort(DATA data_array[], int data_count, string arg, string order);
...
int main(int argc, char *argv[]){
    ...
    if(readCSV(argv[3], &data, &data_count, &start)){
        ...
        bubbleSort(dat, data_count, arg, argv[5]);
    }
    else printf("NO WAY");
...
//reference : https://www.thecrazyprogrammer.com/2011/11/c-program-to-sort-array-by-using-bubble.html
void bubbleSort(DATA data_array[], int data_count, string arg, string order){
    bool swap = false;
    DATA temp;
    while (!swap){
